...
import { 
    ...
    Facebook, Instagram, Youtube, Mail, Phone, Bell, Info
} from 'lucide-react';

// Component Modal Trailer
...
// ✅ Modal Phim Sắp Chiếu
const ComingSoonModal = ({ movie, onClose, isAuthenticated, onBookClick }) => {
    if (!movie) return null;

    const releaseDate = movie.release_date ? new Date(movie.release_date) : null;
    const today = new Date();
    const daysUntilRelease = releaseDate ? Math.ceil((releaseDate - today) / (1000 * 60 * 60 * 24)) : null;
    ...
    const handleBookingClick = () => {
        if (!isAuthenticated) {
            ...
            router.get('/login', {}, {
                onBefore: () => {
                    sessionStorage.setItem('intended_url', `/movies/${movie.id}?date=${movie.release_date}`);
                }
            });
            return;
        }
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-fade-in" onClick={onClose}>
            <div className="relative w-full max-w-4xl bg-gray-900 rounded-2xl overflow-hidden shadow-2xl border border-gray-800" onClick={e => e.stopPropagation()}>
                ...
                {/* Close Button */}
                <button onClick={onClose} className="absolute top-4 right-4 text-white hover:text-red-500 bg-black/50 rounded-full p-2 transition z-30">
                    <X className="w-6 h-6" />
                </button>
                ...
                <div className="relative h-64 overflow-hidden">
                    ...
                    {/* Badge Sắp Chiếu */}
                    <div className="absolute top-4 left-4 bg-yellow-500 text-black px-4 py-2 rounded-full font-bold text-sm shadow-lg flex items-center gap-2">
                ...
                <div className="p-8 -mt-20 relative z-10">
                    <div className="flex gap-6">
                        ...
                        <div className="flex-1">
                            ...
                            {/* Release Date Info */}
                            <div className="bg-gradient-to-r from-red-900/30 to-orange-900/30 border border-red-500/20 rounded-xl p-4 mb-4">
                                <div className="flex items-center gap-3 mb-2">
                                    <CalendarDays className="w-5 h-5 text-red-400" />
                                    <span className="text-sm text-gray-400 font-bold uppercase">Ngày khởi chiếu</span>
                                </div>
                            ...
                            <div className="flex gap-3">
                                {movie.trailer_url && (
                                    <button 
                                        ...
                                        className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 shadow-lg"
                                    >
                                ...
                                {daysUntilRelease !== null && daysUntilRelease <= 7 && (
                                    <button 
                                        onClick={handleBookingClick}
                                        className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 hover:brightness-110 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 shadow-lg"
                                    >
                                        <Ticket className="w-5 h-5" />
                                        {isAuthenticated ? 'Đặt Trước' : 'Đăng nhập để đặt vé'}
                                    </button>
                                )}
                                
                                {(!daysUntilRelease || daysUntilRelease > 7) && (
                                    <button 
                                        className="flex-1 bg-gray-700 text-gray-300 font-bold py-3 rounded-xl flex items-center justify-center gap-2 cursor-not-allowed"
                                        disabled
                                    >
                            ...
                            {/* Info Note */}
                            <div className="mt-4 flex items-start gap-2 text-sm text-gray-400 bg-gray-800/50 p-3 rounded-lg">
                                <Info className="w-4 h-4 mt-0.5 flex-shrink-0" />
                                <p>
                                    {!isAuthenticated 
                                        ? 'Vui lòng đăng nhập để đặt vé xem phim.'
                                        : daysUntilRelease > 7 
                                        ? 'Phim sẽ mở bán vé trước 7 ngày khởi chiếu. Vui lòng quay lại sau!'
                                        : daysUntilRelease > 0
                                        ? 'Bạn có thể đặt trước vé ngay bây giờ. Lịch chiếu chi tiết sẽ cập nhật sớm nhất!'
                                        : 'Phim đã khởi chiếu. Đặt vé ngay!'
                                    }
                                </p>
                            </div>
                        </div>
...
export default function Index({ movies, featureMovie, dateBar, filters }) {
    ...
    const [comingSoonMovie, setComingSoonMovie] = useState(null); // ✅ State cho modal phim sắp chiếu
    ...
    const handleMovieClick = (movie) => {
        ...
        if (movie.status === 'coming_soon') {
            setComingSoonMovie(movie);
            return;
        }

        // Nếu là phim đang chiếu -> Kiểm tra đăng nhập
        checkAuthAndProceed(() => handleQuickBook(movie));
    };
        ...
        <div className="min-h-screen bg-gray-950 text-gray-100 font-sans selection:bg-red-600 selection:text-white" 
            ...
            {comingSoonMovie && (
                <ComingSoonModal 
                    ...
                    onClose={() => setComingSoonMovie(null)}
                    isAuthenticated={!!auth?.user}
                    onBookClick={() => {
                        setComingSoonMovie(null);
                        router.get(`/movies/${comingSoonMovie.id}`, { date: comingSoonMovie.release_date });
                    }}
                />
            )}
                    ...
                    <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide py-2 px-2 snap-x">
                        {shortDateBar.filter(d => d.isSpecial).map((d, i) => (
                            <button
                                ...
                                className={`flex flex-col items-center justify-center min-w-[5rem] h-[6rem] rounded-2xl transition-all duration-300 flex-shrink-0 snap-start relative overflow-hidden ${
                                    d.isActive 
                        ...
                        {shortDateBar.filter(d => !d.isSpecial).map((d, i) => {
                            ...
                            return (
                                <button 
                                    key={i} 
                                    onClick={() => updateFilters({ date: d.fullValue })}
                                    className={`relative flex-1 min-w-[6.5rem] h-[6rem] flex flex-col items-center justify-center rounded-2xl transition-all duration-300 group snap-start overflow-hidden ${
                                        isSelected 
                            ...
                            return (
                                <div key={movie.id} className="group flex flex-col bg-gray-900 rounded-2xl overflow-hidden shadow-lg border border-gray-800 hover:border-gray-600 hover:shadow-red-900/20 transition-all duration-300 hover:-translate-y-2">
                                    <div className="relative aspect-[2/3] overflow-hidden bg-gray-800">
                                        ...
                                        {/* ✅ Badge Sắp Chiếu */}
                                        {isComingSoon && (
                                            <div className="absolute top-2 right-2 z-10 bg-yellow-500 text-black px-2 py-1 rounded text-[10px] font-bold shadow-lg">
                                                SẮP CHIẾU
                                            </div>
                                        )}
                                        ...
                                        <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-3 p-4 backdrop-blur-[2px]">
                                            <button 
                                                ...
                                                className={`w-full font-bold py-2 rounded-lg text-sm text-center shadow-lg transition flex items-center justify-center gap-1 ${
                                                    isComingSoon
